...
/// Storage key discriminants. Composite variants embed all identifying data
/// to guarantee uniqueness across rounds and players.
#[contracttype]
pub enum DataKey {
    Admin,
    PrizePoolContract,
...
impl PatternPuzzle {
    ...
    /// address. Subsequent calls are rejected with `NotAuthorized`.
    pub fn init(
        env:                 Env,
        admin:               Address,
    ...
    pub fn create_puzzle(
        ...
        if env.storage().instance().has(&DataKey::Round(round_id)) {
        ...
        env.storage().instance().set(&DataKey::Players(round_id), &Vec::<Address>::new(&env));
    ...
    ///
    /// Each player may submit exactly once. The `solution` bytes are stored and
    /// compared byte-for-byte against the revealed pattern during `resolve_round`.
    /// Entry fee accounting is updated here; actual token transfer should invoke
    /// the balance contract (see TODO below).
    ...
    /// stored pattern_commitment`. Iterates all submissions to mark winners and
    /// compute `winner_count`. Transitions the round to `Resolved`.
...
mod test {
    ...
    // ------------------------------------------------------------------
    // Bonus: Cannot initialize contract twice
    // ------------------------------------------------------------------
